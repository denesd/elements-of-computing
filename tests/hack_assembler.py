...
class HackAssembler:
    ...
    def __handle_symbols(self, lines):
    ...
    def __translate_dest(self, line):
        result = 0
        if "=" in line:
            dest = line.split("=")[0]
            if "M" in dest:
    ...
    def __handle_instructions(self, lines):
        result = []
        for line in lines:
            if self.__is_a_instruction(line):
    ...
    def __handle_spaces(self, lines):
        return ["".join(line.split()) for line in lines if line.strip()]

    def __handle_comments(self, lines):
...
if __name__ == "__main__":
    ...
    output = filepath.rstrip(".asm") + ".hack"
    assembler = HackAssembler()
    with open(filepath, "r") as input_file:
        code = input_file.read().splitlines()
